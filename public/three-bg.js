...
(function initActiveTheoryHelicalEngine() {
  ...
  const cardData = [
    ...
    },
    {
      id: 'sandbox',
      index: '05',
      category: 'ISOLATED LAB & HARDWARE',
      title: 'SECURITY LAB SANDBOX',
      subtitle: 'VirtualBox & RPi 4 Hardware Lab',
      desc: 'Air-gapped security testbed on VirtualBox Host-Only subnet (192.168.56.0/24) with PCAP packet triage, alongside physical Raspberry Pi 4 Model B GPIO auditing.',
      badge: 'SECURITY LAB',
      color: '#00e5ff',
      stats: [
        { k: 'ISOLATION', v: '100%' },
      ...
      tags: ['VIRTUALBOX', 'RPI 4B', 'PCAP', 'WIRESHARK', 'ARMv8']
    },
    {
      id: 'scholarship',
      index: '06',
      category: 'HONORS & CREDENTIALS',
      title: 'GIAC GFACT CERTIFIED',
      subtitle: 'Foundational Cybersecurity Technologies (GIAC)',
      desc: 'Achieved official GIAC GFACT certification (Issued 1 Sep 2026) via full national scholarship from SANS Institute as a CyberStart Canada Top Performer.',
      ...
      tags: ['GIAC GFACT', 'SANS INSTITUTE', 'CERTIFIED', 'CYBERSTART SCHOLAR']
    },
    {
      id: 'tryhackme',
      index: '07',
  ...
  cardData.forEach((data, i) => {
    ...
    if (i === 0) {
      ...
      // 05: SECURITY LAB // 3D Cybernetic Microchip & Logic Bus Pins
      const chipGeo = new THREE.BoxGeometry(1.8, 1.8, 0.28);
      const chipMat = new THREE.MeshStandardMaterial({ color: 0x0a101f, roughness: 0.3, metalness: 0.9 });
      const chipMesh = new THREE.Mesh(chipGeo, chipMat);

      const dieGeo = new THREE.BoxGeometry(1.0, 1.0, 0.32);
      const dieMat = createHoloMaterial(colHex, true, 0.95);
      const dieMesh = new THREE.Mesh(dieGeo, dieMat);

      // Pins on all 4 sides
      const pinGroup = new THREE.Group();
      const pinMat = new THREE.MeshStandardMaterial({ color: 0xffffff, emissive: colHex, emissiveIntensity: 0.7 });
      for (let p = 0; p < 16; p++) {
        const pinGeo = new THREE.BoxGeometry(0.08, 0.3, 0.08);
        const pin = new THREE.Mesh(pinGeo, pinMat);
        const side = Math.floor(p / 4);
        const offset = (p % 4) * 0.38 - 0.57;
        if (side === 0) pin.position.set(offset, 1.05, 0);
        else if (side === 1) pin.position.set(offset, -1.05, 0);
        else if (side === 2) { pin.rotation.z = Math.PI / 2; pin.position.set(1.05, offset, 0); }
        else { pin.rotation.z = Math.PI / 2; pin.position.set(-1.05, offset, 0); }
        pinGroup.add(pin);
      }

      artifactGroup.add(chipMesh, dieMesh, pinGroup);
      artifactGroup.userData = { type: 'lab', chip: chipMesh, die: dieMesh, pins: pinGroup };

    } else if (i === 5) {
      // 06: SCHOLARSHIP // 3D Cryptographic Octahedron Crystal & Rings
      const octGeo = new THREE.OctahedronGeometry(1.3, 0);
      const octMat = createHoloMaterial(colHex, true, 0.95);
      const octMesh = new THREE.Mesh(octGeo, octMat);

      const innerOctGeo = new THREE.OctahedronGeometry(0.65, 0);
      const innerOctMat = new THREE.MeshStandardMaterial({ color: 0xffffff, emissive: colHex, emissiveIntensity: 1.5, roughness: 0.1 });
      const innerOct = new THREE.Mesh(innerOctGeo, innerOctMat);

      const sealRingGeo = new THREE.TorusGeometry(1.9, 0.03, 16, 64);
      const sealRingMat = createHoloMaterial(0xffb000, false, 0.8);
      const sealRing = new THREE.Mesh(sealRingGeo, sealRingMat);
      sealRing.rotation.x = Math.PI / 3;

      artifactGroup.add(octMesh, innerOct, sealRing);
      artifactGroup.userData = { type: 'scholar', oct: octMesh, innerOct, sealRing };

    } else if (i === 6) {
      // 07: TRYHACKME // 3D Cyber Tesseract (Nested Hypercube)
      ...
      // 08: TRANSMISSION // 3D Quantum Signal Dish & Radiating Pulse Rings
  ...
  function render(now) {
    ...
    targetScroll = Math.max(0, Math.min(8.0, targetScroll));
    ...
    cardMeshes.forEach((mesh) => {
      ...
      if (art && art.userData) {
        ...
        if (u.type === 'identity') {
          ...
          u.chip.rotation.z = Math.sin(timeVal * 1.5) * 0.1;
          u.die.rotation.z = timeVal * 0.5;
          const chipScale = 0.95 + Math.sin(timeVal * 3.0) * 0.05;
          u.die.scale.set(chipScale, chipScale, 1.0);
        } else if (u.type === 'scholar') {
          u.oct.rotation.y = timeVal * 0.5;
          u.oct.rotation.x = Math.sin(timeVal * 0.8) * 0.4;
          u.innerOct.rotation.y = -timeVal * 0.9;
          u.sealRing.rotation.z = timeVal * 0.6;
        } else if (u.type === 'thm') {
    ...
    const tentacleEnergy = (scrollProgress < 0.3 || scrollProgress >= 7.0) ? 1.0 : 0.3;
    ...
    } else if (scrollProgress >= 7.0) {
      const reopenT = Math.min(1.0, (scrollProgress - 7.0) / 1.0);
    ...
    docOckTentacles.forEach((t, tIdx) => {
      ...
      const isCoreHot = (scrollProgress < 0.3 || scrollProgress >= 7.0);
