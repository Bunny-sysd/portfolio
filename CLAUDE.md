...
- Vite 5, used only as a static dev server / build tool — **there is no React, no Tailwind, no `src/` directory.** That entire toolchain (React island, Tailwind, `@react-three/*`, framer-motion, lucide-react — ~1,250 lines across 6 components, none of which ever rendered since their mount targets never existed in `index.html`) was removed. `vite.config.js` has no plugins; `package.json` lists only `vite` and `playwright`.
...
- `style.css` (loaded directly via `<link>`, hand-written, not Tailwind-generated) styles the entire page. Vite's build still auto-hashes it as a build asset since it's referenced via a relative `<link rel="stylesheet">` — expected behavior, not a bug.
