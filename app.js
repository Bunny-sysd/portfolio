...
(function initCanvas() {
  ...
  // Particles (small floating nodes)
  ...
  class Particle {
    ...
    reset() {
      ...
      this.color = Math.random() > 0.6 ? '#00ff88' : Math.random() > 0.5 ? '#00d4ff' : '#a855f7';
    }
    update() {
      this.x += this.vx;
      this.y += this.vy;
  ...
  // Matrix rain drops
  const RAIN_CHARS = '01アイウエオカキクケコサシスセソタチツテト∑∆Ω⟨⟩';
  ...
  function drawMatrix() {
    ...
    for (let i = 0; i < drops.length; i++) {
      ...
      ctx.fillText(RAIN_CHARS[Math.floor(Math.random() * RAIN_CHARS.length)], x, y - 20);
